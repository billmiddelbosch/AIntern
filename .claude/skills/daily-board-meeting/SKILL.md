...
3. End with: `"When all changes are written:\n1. If any .vue or .ts files were modified: run npm run build (from the project root). Fix ALL errors before continuing — do not output the Terminal Summary until the build passes.\n2. Run git diff --stat\n3. Output the full Terminal Summary verbatim:\n\n**Terminal Summary — [Agent] [Action]**\n- Branch: feature/board-{YYYY-MM-DD}\n- Files changed: [list with one-line description per file]\n- Proposed commit message: [message]\n- Diff: [git diff --stat output]\n- Status: ✅ Ready to commit / ❌ Failed ([reason])\n\n4. Ask: **Goedkeuring voor commit? (ja / nee / feedback)**\n5. Wait for the Human Board to respond before committing.\n6. On 'ja': run git add -A && git commit -m '[proposed message]', then output: ✅ Committed.\n7. On 'nee' or feedback: do NOT commit. Output the feedback and close with: ⛔ Commit afgewezen — geen commit gedaan."`
