...
- **security-reviewer agent (2026-07-04 delta re-review)**: 0 CRITICAL/HIGH/MEDIUM/LOW on the shortkey-display and opponent-count-shortcut changes. INFO notes only: the new `shortcut` prop on `PlayingCard.vue` renders via plain-text interpolation (not `v-html`) and is always derived from `cardKey()` over the fixed `Rank`/`Suit` enums, so there's no path for dynamic/user-controlled content; the two unscoped global `keydown` listeners (`PokerOddsCalculator.vue` for `/`/`°`, pre-existing one in `CardPickerGrid.vue` for rank/suit chars) don't overlap character sets and both correctly skip form fields/modifier keys and auto-detach on unmount via VueUse; `setOpponents`'s `Math.min`/`Math.max` clamp is correct at both bounds and isn't exported/externally callable with untrusted input today.
