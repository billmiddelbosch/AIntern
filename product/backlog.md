...
| B-45 | **Kennisbank artikel — "Software bouwen is skills en agents bouwen"** | S | CMO | todo | board-meeting-2026-04-23 | Week 17 artikel 2 — seed: Obsidian "2026-04-20 Software bouwen is skills en agents bouwen.md". Gepubliceerd op S3; sitemap geregenereerd; week 17 Kennisbank KPI 2/2 ✅. |
| B-46 | **S-02: OG/Twitter Card meta-tags homepage** | S | CTO | todo | board-meeting-2026-04-23 | `og:image`, `og:url`, `og:title`, `og:description`, `twitter:card`, `twitter:title`, `twitter:description` toegevoegd via useHead() in App.vue of HomeView.vue; verwijst naar bestaande `public/og-image.png`; type-check pass. |
| B-47 | **Wekelijkse security check week 17** | S | CTO | todo | board-meeting-2026-04-23 | Security rapport gedocumenteerd in `.claude/cto/memory_security_check_2026-04-23.md`; PASS of findings geëscaleerd naar CEO. |
...
| B-49 | **Leads naar DynamoDB + list view + edit/copy in /admin/leads** | M | CTO | todo | board-meeting-2026-04-23 | (1) Lambda lead-crud.ts: DynamoDB CRUD — GET alle leads, GET by id, PUT update velden. (2) CDK: leadCrudFn DynamoDB permissions + DYNAMODB_TABLE_SSM_PREFIX env + PUT /admin/leads/{id} route. (3) Import script: seeds DynamoDB vanuit outreach-log.csv (eenmalig). (4) Vue: AdminLeadFormView.vue — edit form met copy-knoppen voor connection/DM berichten + save via PUT. (5) Router: /admin/leads/:id route. (6) LeadCard klikt naar form. In lijn met admin/kennisbank en admin/linkedin. |
