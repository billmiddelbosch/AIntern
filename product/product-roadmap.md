...
### 11. AI Studio — Component Builder & Template Builder (planned)
Intern admin-hulpmiddel voor het genereren, previeuwen en opslaan van Vue-componenten en paginatemplates via Claude AI. Twee builders achter `/admin/ai-studio`: de Component Builder produceert herbruikbare `.vue` SFC's en slaat ze op in `src/components/ai-generated/`; de Template Builder produceert volledige Kennisbank-paginatemplates en slaat ze op in `src/views/`. Live sandbox-preview in een iframe via Vue runtime compiler. Opslaan verloopt via Lambda → GitHub API commit (PAT in AWS Secrets Manager). Config-gestuurde actieve template via DynamoDB. Galerij toont eerder gegenereerde items. Primair gebruik: interne snelheid + client-demo's.
